'''
Create a function that returns the number of times a character appears
in each word in a sentence. Treat upper and lower case characters of
the same letter as being identical (e.g. a exists in Anna twice, not once).
'''

def char_appears(txt, char):
    return [i.lower().count(char) for i in txt.split(' ')]
